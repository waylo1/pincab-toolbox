...
public static class ScenariosTests
{
    ...
    {
        var matches = Scenarios.DetectAll(Codes(), fr: false);
        Assert.Equal(0, matches.Count);
    }

    public static void Test_Unrelated_Codes_Yield_No_Matches()
    {
        var matches = Scenarios.DetectAll(Codes("SOME_UNRELATED_CODE", "ANOTHER_ONE"), fr: false);
        Assert.Equal(0, matches.Count);
    }

    public static void Test_VpinmameNotRegistered_Alone_Is_Enough_MinMatch_1()
    {
        var matches = Scenarios.DetectAll(Codes("VPINMAME_NOT_REGISTERED"), fr: false);
    ...
    public static void Test_BitnessMigration_Requires_At_Least_Two_Of_Its_Three_Codes()
    {
        ...
        var matches = Scenarios.DetectAll(Codes("BITNESS_MISMATCH_VPM"), fr: false);
    ...
    {
        var matches = Scenarios.DetectAll(Codes("BITNESS_MISMATCH_VPM", "BITNESS_DMD64_MISSING"), fr: false);
        Assert.Equal(1, matches.Count);
    ...
    {
        var oneCode = Scenarios.DetectAll(Codes("POPPER_NOT_REGISTERED"), fr: false);
        Assert.Equal(0, oneCode.Count);

        var bothCodes = Scenarios.DetectAll(Codes("POPPER_NOT_REGISTERED", "B2S_MISSING"), fr: false);
    ...
    public static void Test_Chain_Only_Shows_Steps_Whose_Backing_Code_Actually_Matched()
    {
        ...
        var matches = Scenarios.DetectAll(Codes("BITNESS_MISMATCH_VPM", "BITNESS_HYBRID_INSTALL"), fr: false);
        Assert.Equal(1, matches.Count);
    ...
    {
        var two = Scenarios.DetectAll(Codes("BITNESS_MISMATCH_VPM", "BITNESS_DMD64_MISSING"), fr: false)[0].Confidence;
        var three = Scenarios.DetectAll(Codes("BITNESS_MISMATCH_VPM", "BITNESS_DMD64_MISSING", "BITNESS_HYBRID_INSTALL"), fr: false)[0].Confidence;
    ...
    public static void Test_Confidence_Is_Capped_At_96()
    {
        ...
        var matches = Scenarios.DetectAll(Codes("BITNESS_MISMATCH_VPM", "BITNESS_DMD64_MISSING", "BITNESS_HYBRID_INSTALL"), fr: false);
        Assert.Equal(96, matches[0].Confidence);
    }

    public static void Test_Codes_Outside_Any_Scenario_Do_Not_Inflate_Confidence()
    {
        var withoutNoise = Scenarios.DetectAll(Codes("VPINMAME_NOT_REGISTERED"), fr: false)[0].Confidence;
        var withNoise = Scenarios.DetectAll(Codes("VPINMAME_NOT_REGISTERED", "SOME_UNRELATED_CODE"), fr: false)[0].Confidence;
    ...
    public static void Test_Multiple_Scenarios_Fire_Together_When_All_Their_Codes_Co_Occur()
    {
        var matches = Scenarios.DetectAll(Codes(
            ...
            "VPINMAME_NOT_REGISTERED"), fr: false);
    ...
    {
        Assert.Equal(null, Scenarios.Detect(Codes(), fr: false));
    }
    ...
    {
        var m = Scenarios.DetectAll(Codes("VPINMAME_NOT_REGISTERED"), fr: true)[0];
    ...
    {
        var m = Scenarios.DetectAll(Codes("VPINMAME_NOT_REGISTERED"), fr: false)[0];
    ...
    public static void Test_TriggeredBy_Lists_Exactly_The_Codes_That_Matched_Not_All_Scenario_Codes()
    {
        ...
        var matches = Scenarios.DetectAll(Codes("POPPER_NOT_REGISTERED", "B2S_MISSING", "UNRELATED"), fr: false);
    ...
    {
        var matches = Scenarios.DetectAll(Codes("BITNESS_MISMATCH_VPM32"), fr: false);
        Assert.Equal(1, matches.Count);
    ...
    {
        var matches = Scenarios.DetectAll(Codes("COM_STALE_PATH"), fr: false);
        Assert.Equal(1, matches.Count);
    ...
    {
        var matches = Scenarios.DetectAll(Codes("ALTSOUND_PRESENT_NOT_ENABLED"), fr: false);
        Assert.Equal(1, matches.Count);
    ...
    {
        var matches = Scenarios.DetectAll(Codes("ALTCOLOR_PRESENT_NOT_ENABLED"), fr: false);
        Assert.Equal(1, matches.Count);
    ...
    {
        var matches = Scenarios.DetectAll(Codes("ALTSOUND_PRESENT_NOT_ENABLED", "ALTCOLOR_PRESENT_NOT_ENABLED"), fr: false);
        Assert.Equal(1, matches.Count);
    ...
    {
        var one = Scenarios.DetectAll(Codes("ALTSOUND_PRESENT_NOT_ENABLED"), fr: false)[0].Confidence;
        var two = Scenarios.DetectAll(Codes("ALTSOUND_PRESENT_NOT_ENABLED", "ALTCOLOR_PRESENT_NOT_ENABLED"), fr: false)[0].Confidence;
    ...
    {
        var vpm32 = Scenarios.DetectAll(Codes("BITNESS_MISMATCH_VPM32"), fr: true)[0];
        Assert.Equal("Installation 32-bit avec VPinMAME 64-bit uniquement", vpm32.Title);

        var stale = Scenarios.DetectAll(Codes("COM_STALE_PATH"), fr: true)[0];
        Assert.Equal("Composant enregistré vers un emplacement supprimé", stale.Title);

        var extras = Scenarios.DetectAll(Codes("ALTSOUND_PRESENT_NOT_ENABLED"), fr: true)[0];
    ...
    public static void Test_Point4_Scenarios_Can_All_Fire_Alongside_The_Original_Three()
    {
        var matches = Scenarios.DetectAll(Codes(
            ...
            "BITNESS_MISMATCH_VPM32", "COM_STALE_PATH", "ALTSOUND_PRESENT_NOT_ENABLED"), fr: false);

...
## 2026-08-20 (suite, même jour) · Repositionnement DMD hors-écran + mode Débutant/Expert — « termine CE chantier », revue CTO du 20/08 exécutée intégralement
- code:        DMD_POSITION_OFFSCREEN (nouvelle règle) — aucun code pour le mode Débutant/Expert (UI transverse au panneau de détail)
- bac:         FEATURE
- contexte:    après la revue CTO+Product de la feuille de route de GPT (voir brief `docs/BRIEF-pour-gpt-2026-08-20.md`), Maxime a explicitement demandé de tout implémenter sans attendre une session future : « non je disais qu'il fallait terminer CE chantier donc tout ce que tu viens de dire il faut le faire, go »
- analyse:     deux items retenus comme raisonnablement bornés pour ce chantier (le reste — snapshot local — reste plus lourd, voir disposition). (1) DMD_POSITION_OFFSCREEN avait déjà toute l'infrastructure de lecture (`DmdDeviceIniParser.TryParseVirtualDmdConfig`) mais aucune action d'écriture — repositionner en (0,0) est réversible par construction (l'ancienne position est connue avant écriture), contrairement à `register_com_component`. (2) le mode Débutant/Expert coûte moins cher que l'estimation de GPT : le Knowledge Pack sépare déjà le texte simple (`PackKnowledge.Player`, utilisé dans "Symptôme") du texte technique (`Knowledge.Impact/Cause`, `PackKnowledge.Explanation/Verification`) — il suffisait de conditionner l'affichage de 4 sections déjà existantes, pas de dupliquer le contenu.
- disposition: ✅ codé et testé le 20/08. **(1) DMD** : `DmdDeviceIniParser.RewriteVirtualDmdPosition` (Core, réécrit uniquement `left`/`top`, préserve tout le reste du fichier ligne par ligne, jamais n'invente une section `[virtualdmd]` absente) + `RepositionDmdAction` (`reposition_dmd`, `IsReversibleByNature=true`, l'ancienne position est encodée dans `PlannedChange.Before` et reparsée par son propre `Revert()`) + règle pack (confiance 85, réversible, sauvegarde requise). Enregistrée dans les deux `RepairActionRegistry` (App + Repair). 10 tests dédiés à l'action + 5 tests du rewriter + 1 test de bout en bout sur le vrai pack (plan→apply→undo, position `5000,5000`→`0,0`→`5000,5000`). **(2) Mode Débutant/Expert** : nouveau bouton `BtnExpertMode` (même emplacement/pattern que les 3 boutons de langue), persisté dans `Settings.ExpertMode` (défaut `true` = Expert, délibéré pour ne rien changer silencieusement à l'expérience des testeurs déjà en cours de test). En mode Débutant, `ListFindings_SelectionChanged` masque Impact/Cause/Explication/Vérification (texte technique) et ne garde que Symptôme+Correctif (déjà en langage simple) — réutilise le mécanisme de collapse existant (`SetSection(..., null)`), aucune nouvelle logique d'affichage. Bascule immédiate (pas besoin de rouvrir un finding), persistée tout de suite comme la langue. Clés `mode.expert`/`mode.beginner`/tooltips ajoutées en FR/EN/ES. **721 tests Core+Repair verts en sandbox (build+run confirmés). Toute la partie App/WPF (bouton, XAML, Settings, MainWindow.xaml.cs) est non compilable en sandbox (pas de compilateur Windows ici) — à confirmer par `build.cmd` avant de faire confiance à l'affichage, même disclosure que le reste de la session.** Non traité dans ce lot : le "Snapshot local" (comparaison de config avant/après) — scope nettement plus large (nouveau modèle de stockage, nouvel écran, moteur de comparaison), volontairement laissé en dehors de "termine CE chantier" en l'absence de confirmation explicite sur ce point précis ; décision de séquencement à confirmer avec Maxime.

## 2026-08-20 (suite, même jour) · Joey Mahon — B2S cassé en renommant l'ancien dossier d'install, le scanner n'a rien montré
- code:        COM_PATH_OUTSIDE_INSTALL, COM_STALE_PATH — pas de code concerné, comportement déjà existant relu à la lumière de ce retour
- bac:         FN (signal probable, non recoupé)
- contexte:    Joey a deux installs VPX/Popper sur la même machine (ancienne + nouvelle, migration de tables en cours). En renommant le dossier de l'ANCIENNE install pour préparer sa suppression, la NOUVELLE install (qu'il croyait indépendante et déjà fonctionnelle) s'est cassée : B2S.Server chargeait en fait la copie de l'ancien dossier depuis le début. Verbatim : « the B2S.Server having 2 different files showing registered in the Input 32 section of the registry [...] the tool didn't show anything in it that was critical or broken. » Il propose aussi : détecter les « doublons » dans le registre.
- analyse:     techniquement cohérent avec `ComHealthScanner` tel qu'il existe : l'enregistrement COM Windows d'un ProgID est GLOBAL à la machine, pas par install — un seul chemin vit dans le registre par vue 32/64-bit à un instant donné (pas de vraie notion de « doublon simultané », Joey voit sans doute deux valeurs différentes en comparant un export avant/après son changement, pas deux entrées coexistantes). Ce que Joey a probablement vécu, chemin par chemin dans `EvaluateComponent` : le ProgID B2S.Server était enregistré vers le dossier de l'ANCIENNE install ; `IsUnderRoot` (comparé à la racine scannée = la NOUVELLE install) renvoie faux → branche `COM_PATH_OUTSIDE_INSTALL`. **Deux issues possibles selon que sa nouvelle install a ou non sa propre copie locale de B2S.Server** : (a) si oui, `COM_PATH_OUTSIDE_INSTALL` se déclenche mais en `Severity.Note` — délibérément neutre, n'affecte pas le score, facile à ne pas remarquer comme « broken » (ses mots exacts) ; (b) si non, la branche `else` du code se tait complètement (commentaire du 10/08 : « a legitimate other install with nothing local to compare against — silence, per spec A.2 » — multi-install n'est pas en soi un défaut). Le cas (b) correspond exactement à « the tool didn't show anything » si sa nouvelle install n'a pas sa propre copie de B2S. **Le point faible que révèle ce retour** : l'hypothèse du 10/08 (« un autre install, rien à comparer, silence ») suppose que cet autre dossier est un choix délibéré et stable — le cas de Joey montre qu'il peut aussi s'agir d'un dossier en cours de suppression, invisible pour le scanner tant qu'il n'a pas déjà cassé quelque chose. La suggestion de Joey (« doublons du registre ») n'est pas littéralement possible (le registre ne garde qu'une valeur courante par ProgID/vue), mais la version réellement utile de son idée existe déjà en partie : c'est `COM_PATH_OUTSIDE_INSTALL`, seulement trop silencieux/trop neutre pour ce cas précis.
- disposition: **non codé, signal isolé (un seul testeur, un seul cas)** — consigné pour attendre un deuxième signal avant de toucher au comportement de silence, cohérent avec la discipline du projet (un FP ajouté par sur-réaction coûte plus cher qu'un FN documenté). Piste déjà identifiée si un deuxième cas arrive : arrêter de taire la branche « pas de copie locale » de `COM_PATH_OUTSIDE_INSTALL` — même sans copie locale à comparer, le fait que le composant tourne depuis un dossier hors de CET install est en soi une fragilité (« si ce dossier bouge ou disparaît, ça casse »), qui mérite au moins un Note. Risque à peser avant de coder : combien d'installs légitimement partagées (VPinMAME/B2S installé une fois pour plusieurs cabs) verraient apparaître un Note qui n'apporte rien de plus qu'aujourd'hui. **Réponse à Joey en attente de Maxime** : il demande aussi si Maxime utilise des pup packs sur sa propre cab, question personnelle à laquelle seul Maxime peut répondre.

## 2026-08-20 (suite, même jour) · Boris France — ratio ROM détecté quasi inversé sur 100 tables, Die Hard en faux ROM_MISSING
- code:        ROM_NOT_REQUIRED / ROM_MISSING (RomValidatorScanner) — signal aussi mentionné sur B2S/flexdmd et sur le message "à jour", pas encore analysable sans son rapport
- bac:         FN probable (à confirmer)
- contexte:    post d'annonce Facebook (groupe Pincab Passion), commentaire de Boris France, 100 tables scannées
- verbatim:    « Sur mes 100 tables, l'outil me dit qu'il y en a quasi aucune qui nécessite une rom (c'est tout le contraire). Par contre pour Die Hard, il me dit qu'il manque la rom alors que ça fonctionne très bien. Il y a 2 ou 3 autres trucs avec le B2S et flexdmd qui semble remonter des alertes [...] Et l'outil me dit que toutes mes tables sont à jours ! C'est certains que ça n'est pas le cas. »
- analyse:     trois signaux distincts dans ce retour, pas un seul bug. (1) **ratio ROM_NOT_REQUIRED quasi inversé sur 100 tables** — RomValidatorScanner (ligne 68) route en `ROM_NOT_REQUIRED` (Ok) dès que `!rom.UsesController || rom.Candidates.Count == 0`. Si `ScriptAnalyzer.AnalyzeRomUsage` échoue à détecter `UsesController` sur son style de scripts (regex trop stricte, syntaxe VBS particulière à sa collection, ou tables d'un pack précis), la quasi-totalité de ses tables pilotant réellement VPinMAME sortirait à tort comme "pas de ROM requise" — cohérent avec ce qu'il décrit, mais NON confirmé sans voir un script qui échoue. (2) **Die Hard en ROM_MISSING alors que ça fonctionne** — cas inverse et plus rare : soit un nom de ROM candidat différent du nom réellement chargé par VPinMAME (VPMAlias non résolu, ou candidat mal extrait du script), soit le jeu fonctionne via un mécanisme hors du scope de `ctx.RomSets` (ROM dans un sous-dossier non scanné, ou chargée autrement). Anomalie isolée dans son propre message (il dit lui-même "à l'inverse" du problème global), donc probablement une cause différente du (1). (3) **"toutes mes tables sont à jour"** — très probablement une confusion avec `about.update.uptodate` ("Vous êtes à jour"), qui compare la version de L'APPLI Pincab Toolbox à GitHub, PAS une comparaison table par table (ça, c'est `VPS_MATCH_SUMMARY`/`UPDATE_AVAILABLE`, qui annonce explicitement son taux de reconnaissance et les mods non comparés). Pas un bug si c'est bien ce message qu'il a lu — mais wording à clarifier si un deuxième testeur fait la même confusion. B2S/flexdmd : trop vague pour analyser, il dit lui-même vouloir regarder plus précisément.
- disposition: **non codé, signal isolé pas encore assez précis** — (1) est potentiellement le plus grave des trois (touche le cœur du scanner ROM sur toute une collection), mais impossible à corriger à l'aveugle sans un script VPS qui reproduit l'échec de détection : corriger sans repro risquerait de rouvrir KPI#1 (faux ROM_MISSING sur tables originales, FIELD-LOG 2026-07-29/30) dans l'autre sens. **Réponse à Boris : lui demander son rapport exporté** (comme Joey) pour identifier précisément quelles tables/scripts font échouer `AnalyzeRomUsage`, exactement la même discipline que le cas Joey ci-dessus.

## 2026-08-21 · crrispy — B2S.Server et FlexDMD.FlexDMD signalés « pointent vers mscoree.dll, n'existe plus » alors que ça fonctionne, faux positif confirmé et codé
- code:         COM_STALE_PATH (ComHealthScanner) — corrigé
- bac:          FP, confirmé par lecture directe du code, pas seulement par le retour terrain
- contexte:     commentaire public, crrispy a scanné tout son C: et reçu deux findings Warning sur B2S.Server et FlexDMD.FlexDMD (« pointe vers mscoree.dll, qui n'existe plus — reste d'une ancienne install, le chargement échouera ») alors que les deux fonctionnent très bien chez lui.
- verbatim:     « Ça, ça m'a l'air d'être du faux positif ? Cette dll est dans les répertoires Windows system qui sont dans le path, c'est du .net, B2S server et FlexDMD fonctionnent très bien. »
- analyse:      crrispy a raison, et ce n'est pas un cas limite, c'est la forme NORMALE d'enregistrement de B2S.Server et FlexDMD.FlexDMD. Les deux sont des classes COM .NET enregistrées à la manière standard de regasm (sans `/codebase`) : la valeur par défaut de `InprocServer32` ne pointe jamais vers l'assembly lui-même, elle pointe vers l'hébergeur CLR générique, enregistré comme le nom de fichier nu `mscoree.dll`, sans dossier. Windows le résout lui-même via son ordre de recherche système (System32/SysWOW64) au moment de l'activation COM, jamais via le dossier courant du process appelant. `ComHealthScanner.EvaluateComponent` appelait `fileExists(preferred.ServerPath)` directement sur ce nom nu — `File.Exists("mscoree.dll")` ne regarde que le dossier de travail de l'appli, jamais System32, donc il renvoyait systématiquement faux, et TOUTE install saine de B2S/FlexDMD recevait ce faux Warning. Confirmé en lisant `ComRegistrationProbe.ReadServerPath` (lit la valeur brute de la clé, jamais résolue) et `ComHealthScanner.cs` ligne ~214 (l'ancien `fileExists(preferred!.ServerPath)`).
- disposition:  **codé et testé, pas un simple signal isolé** — root cause confirmée par lecture de code, pas seulement déduite d'un rapport terrain, donc pas soumis à la règle des deux signaux. Fix : `IsBareDotNetComHost` détecte le nom nu `mscoree.dll` sans dossier, `ResolveKnownDllPath` le résout vers `%windir%\System32` (vue 64 bits) ou `%windir%\SysWOW64` (vue 32 bits) avant de tester son existence — c'est la résolution RÉELLE de Windows, pas une supposition. Quand ça résout et que le fichier existe bien, le scanner reste **silencieux** plutôt que d'inventer un verdict "à l'intérieur" ou "à l'extérieur" de l'install (les valeurs Assembly/CodeBase qui donneraient la vraie cible ne sont pas lues par cette sonde) — cohérent avec la doctrine déjà en place ailleurs dans ce scanner ("ne jamais deviner, se taire"). 3 tests dédiés (`ComHealthScannerTests`, cas résolu+présent → silencieux, cas vraiment absent → COM_STALE_PATH inchangé, chemin déjà complet → comportement d'origine inchangé). Suites Core (548/548) et Repair (176/176) vertes après le fix. Réponse à crrispy : confirmé et corrigé, merci pour les deux lignes précises du rapport qui ont permis de retrouver la cause exacte sans avoir besoin d'un deuxième signal.
...
- ~~Séparateur décimal Windows changé temporairement pour un fix~~ — **écarté, décision architecture (20/08)**. Idée proposée par GPT (fil `docs/BRIEF-pour-gpt-2026-08-20.md`) pour un scénario de config mal parsée à cause de la virgule/point régional. Rejetée en revue CTO : c'est un réglage Windows **système**, pas confiné à la racine d'install détectée — incompatible avec ADR-005 (toute écriture de Repair reste confinée aux racines d'install). Le scanner peut continuer à **détecter** ce cas (signal informatif), mais aucune action Repair ne doit jamais toucher un réglage système global. Reste en détection-seule si un jour codé ; jamais en auto-fix.
